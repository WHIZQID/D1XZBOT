...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:D1XZBOT\n'
            + 'ORG:Owner BOT;\n'
            + 'TEL;type=CELL;type=VOICE;waid=62853369666811:+62 853-3696-6681\n'
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["62853369666811@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["62853369666811@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					...
					if (isUser) return reply('kamu sudah Menjadi Temen D1XZBOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('62853369666811@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 D1XZ BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 D1XZ BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat D1XZBOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *D1XZBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
     ...
     reply(mess.wait)
		...
		case 'jadwalsholat':
           ...
           case 'jadwaltvnow':
                ...
                if (!isUser) return reply(mess.only.userB)
                if (isLimit(sender)) return reply(limitend(pushname2))
                anu = await fetchJson(`https://api-melodicxt.herokuapp.com/api/jadwaltvnow?&apiKey=administrator`, {method: 'get'})
...
// premium user
       ...
       case 'ytmp4':
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *TIMELINE PLAY MP3* 」\n*• Judul:* ${anu.result.title}\n*• Source:* ${anu.result.source}\n*• Ukuran:* ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA GAESS*`
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[D1XZBOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
